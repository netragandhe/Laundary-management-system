...
router.post('/', authenticate, requirePermission('manage_staff'), async (req, res) => {
  try {
    ...
    // Capture plain password BEFORE save hashes it
    const plainPassword = password;
    await user.save();

    // Send welcome email with login credentials
    try {
      const branchNames = validBranchObjectIds.length > 0
        ? (await Branch.find({ _id: { $in: validBranchObjectIds } })).map(b => b.name).join(', ')
        : '';
